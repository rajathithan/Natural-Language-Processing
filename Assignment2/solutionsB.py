...
def split_wordtags(brown_train):
    ...
    for sentence in brown_train:
        ...
        brown_words.append(sentWords)
        brown_tags.append(sentTags)
...
def calc_trigrams(brown_tags):
    q_values = {}
    biCountMap = {}
    sentCount = len(brown_tags)
    for sentTags in brown_tags:
        for i, tag in enumerate(sentTags):
            ...
            if(i >= 2):
                ...
                q_values[triTuple] = q_values.get(triTuple, 0) + 1
    ...
    for tri in q_values:
        ...
        q_values[tri] = math.log(q_values[tri],2) - math.log(base,2)
...
def calc_emission(brown_words_rare, brown_tags):
    # set e_values to be a default integer value dictionary that stores the emission probability of every (word, tag) pair
    e_values = collections.defaultdict(int)
    ...
    for i in range(len(brown_tags)):
        ...
        for j in range(len(brown_tags[i])):
            # update tag count and (word, tag) pair count
            tag_c[brown_tags[i][j]] += 1
            e_values[(brown_words_rare[i][j], brown_tags[i][j])] += 1
    # for each entry in e_values dictionary, calculate (word, tag) probability by dividing corresponding tag counts
    for k,v in e_values.items():
        e_values[k] = math.log(float(v)/tag_c[k[1]], 2)
